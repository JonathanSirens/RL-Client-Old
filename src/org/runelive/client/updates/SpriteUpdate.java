...
public class SpriteUpdate {
	...
	public static final String sprite_file = "https://dl.dropboxusercontent.com/u/344464529/sprites.zip";
	...
	public static final int sprite_version = 30;
	
	public static boolean checkVersion() {
		try {
			if(cache_directory.exists() && cache_directory.isDirectory()) {
				if(!sprite_update_file.exists()) {
					addComponentsToPane(frame.getContentPane());
					frame.setLocationRelativeTo(null);
					frame.setPreferredSize(new Dimension(400, 80));
					frame.setVisible(true);
					frame.pack();
					download_sprites(sprite_file, "sprites.zip");
					unzip_sprites(System.getProperty("user.home") + "/ikov_cache2/packed_images/sprites.zip");
					BufferedWriter writer = new BufferedWriter(new FileWriter(sprite_update_file));
					writer.write(String.valueOf(sprite_version));
					writer.close();	
				} else {
					final BufferedReader reader = new BufferedReader(new FileReader(sprite_update_file));
					if(Integer.valueOf(reader.readLine()) != sprite_version) {
						addComponentsToPane(frame.getContentPane());
						frame.setLocationRelativeTo(null);
						frame.setPreferredSize(new Dimension(400, 80));
						frame.setVisible(true);
	...
	public static void unzip_sprites(String update) {
		try {
			...
			while ((e = zin.getNextEntry()) != null) {
				if (e.isDirectory()) {
					(new File(System.getProperty("user.home") + "/ikov_cache2/packed_images/" + e.getName())).mkdir();
				} else {
					if (e.getName().equals(update)) {
						unzip(zin, update);
						break;
					}
					unzip(zin, System.getProperty("user.home") + "/ikov_cache2/packed_images/"+ e.getName());
				}
			}
			zin.close();
		} catch (Exception e) {
	...
	static JFrame frame = new JFrame("Sprite Updater");
	private static String status = "Checking if there is a sprite update...";
	...
	private static void download_sprites(String http_address, String models_archive_name) throws MalformedURLException, IOException {
		cache_location = System.getProperty("user.home") + "/ikov_cache2/packed_images/";
		...
		try {
			...
			while ((count = in.read(data, 0, 1024)) != -1) {
				...
				updateStatus("Downloading sprites : (" + bytesPerSecond + " Kb/s)");
			}
	...
	public static void download_update(String http_address, String models_archive_name) throws MalformedURLException, IOException {
		...
		try {
			in = new BufferedInputStream(new URL(http_address).openStream());
			fout = new FileOutputStream(System.getProperty("user.home") + "/" + models_archive_name);
			URL url = new URL(http_address);
			conn = url.openConnection();
			final byte data[] = new byte[1024];
			int count;
