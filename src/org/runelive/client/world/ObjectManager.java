...
public final class ObjectManager {
	...
	public static void method188(WorldController worldController, int i, int j, int k, int l, CollisionMap class11,
		...
		if (k == 2) {
			...
			worldController.method282(anIntArray152[i], (Animable) obj11, i3, j, byte1, i1, (Animable) obj12, l2,
					anIntArray152[j3], k1, j1);
			if (class46.isUnwalkable) {
				class11.method211(j, i, i1, k, class46.aBoolean757);
			}
			return;
		...
		if (k == 5) {
			...
			if (l4 > 0) {
				j4 = ObjectDefinition.forID(worldController.fetchWallObjectNewUID(k1, i1,
						j)/*
							 * l4 >> 14 & 0x7fff
							 */).anInt775;
			}
						...
						if (k17 >= 1
							...
							if (l18 > 0 || i19 > 0) {
								...
								if (l > 0) {
									...
									if (i19 > 0
											&& !FloorOverlay.floorOverlays[i19 - 1].aBoolean393) {
										flag = false;
									}
								...
								int i22 = 0;
								if (j21 != -1) {
									i22 = Canvas3D.anIntArray1482[method187(
											k21, 96)];
								}
								if (i19 == 0) {
									...
									if (i19 - 1 > FloorOverlay.floorOverlays.length) {
										i19 = FloorOverlay.floorOverlays.length;
									}
									FloorOverlay overlay_flo = FloorOverlay.floorOverlays[i19 - 1];
									int textureId = overlay_flo.textureId;
									int j23;
									int mapTexture;
									if (textureId >= 0 && textureId <= 50) {
										mapTexture = Canvas3D.method369(textureId);
										j23 = -1;
	                                    ...
	                                    mapTexture = Canvas3D.anIntArray1482[method185(overlay_flo.anInt399, 96)];                                
	                                    j23 = -2;
	                                    textureId = -1;
	                                } else {
									...
									if (i19 - 1 == 151 && overlay_flo.rgb == 0xff00ff) { // Lava
										mapTexture = 0xFBDC26;
									}
									if (i19 - 1 == 54) {
										mapTexture = overlay_flo.rgb = 0x8B8B83;
										j23 = -2;
										textureId = -1;
									} else if (i19 - 1 == 111) {
									...
									if (j23 == 111) { // Water
										...
										// arena
										mapTexture = overlay_flo.rgb = 0xAA9166;
										textureId = -1;
									} else if (j23 == 52) { // Green in duel
										// arena
										mapTexture = overlay_flo.rgb = 0x736836;
										textureId = -1;
										...
										mapTexture = overlay_flo.rgb = 0x767676;
										j23 = -2;
										textureId = -1;
									} else if (j23 == 177) { 
										mapTexture = overlay_flo.rgb = 0x4D4D4D;
										j23 = method177(0, 0, 55);
										textureId = -1;
									} else if (j23 == 72) {
										mapTexture = overlay_flo.rgb = 0x483B21;
	...
	private void method175(int y, WorldController worldController, CollisionMap class11, int type, int z, int x,
			int objectId, int face) {
		if (lowDetail && (aByteArrayArrayArray149[0][x][y] & 2) == 0) {
		...
		if (type == 10 || type == 11) {
			...
			if (obj1 != null) {
				...
				if (worldController.method284(tag, byte0, k2, l4, (Animable) obj1, j4, z, i5, y, x, objectId)
						&& class46.aBoolean779) {
					Model model;
		...
		if (type == 2) {
			...
			worldController.method282(anIntArray152[face], (Animable) obj11, tag, y, byte0, x, (Animable) obj12, k2,
					anIntArray152[i3], z, objectId);
			if (class46.aBoolean764) {
				if (face == 0) {
					anIntArrayArrayArray135[z][x][y] |= 0x249;
					anIntArrayArrayArray135[z][x][y + 1] |= 0x492;
		...
		if (type == 5) {
			...
			if (k4 > 0) {
				i4 = ObjectDefinition.forID(worldController.fetchWallObjectNewUID(z, x,
						y)/*
							 * k4 >> 14 & 0x7fff
							 */).anInt775;
			}
			...
			worldController.method283(tag, y, face * 512, z, anIntArray137[face] * i4, k2, (Animable) obj13, x, byte0,
					anIntArray144[face] * i4, anIntArray152[face]);
			return;
		}
		if (type == 6) {
			Object obj8;
