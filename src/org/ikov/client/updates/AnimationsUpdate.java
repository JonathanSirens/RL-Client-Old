...
public class AnimationsUpdate {

	private static final int BUFFER = 1024;
	
	private static String cache_location;
	
	public static final String animation_file = "https://dl.dropboxusercontent.com/u/344464529/index2.zip";
	public static final File animation_update_file = new File(System.getProperty("user.home") + "/ikov_cache2/versions/animations_version");
	...
	public static boolean checkVersion() {
		try {
			if(cache_directory.exists() && cache_directory.isDirectory()) {
				if(!animation_update_file.exists()) {
					...
					frame.pack();
					REPACK = 1;
					download_animations(animation_file, "index2.zip");
					unzip_animations(System.getProperty("user.home") + "/ikov_cache2/cache_index_data/index2.zip");
					BufferedWriter writer = new BufferedWriter(new FileWriter(animation_update_file));
					writer.write(String.valueOf(animation_version));
					writer.close();	
					BufferedWriter writer2 = new BufferedWriter(new FileWriter(animation_repack_file));
					writer2.write(String.valueOf(REPACK));
					writer2.close();
				} else {
					final BufferedReader reader = new BufferedReader(new FileReader(animation_update_file));
					if(Integer.valueOf(reader.readLine()) != animation_version) {
						addComponentsToPane(frame.getContentPane());
						frame.setLocationRelativeTo(null);
						frame.setPreferredSize(new Dimension(400, 80));
						frame.setVisible(true);
	...
	public static void unzip_animations(String update) {
		try {
	...
	static JFrame frame = new JFrame("Animations Updater");
	private static String status = "Checking if there is a animations update...";
	...
	private static void download_animations(String http_address, String models_archive_name) throws MalformedURLException, IOException {
		...
		try {
			...
			while ((count = in.read(data, 0, 1024)) != -1) {
				...
				updateStatus("Downloading animations : (" + bytesPerSecond + " Kb/s)");
			}
