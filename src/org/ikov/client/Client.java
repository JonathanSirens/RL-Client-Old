...
import org.ikov.client.RandomColor;
import org.ikov.alertify.Alertify;
import org.ikov.alertify.AlertifyBuilder;
import java.io.FileInputStream;
import java.io.FileOutputStream;
import java.util.zip.GZIPOutputStream;
import org.ikov.alertify.AlertifyType;
import org.ikov.alertify.ui.*;
import java.awt.Color;
import java.awt.Component;
import java.awt.RenderingHints;
import org.ikov.client.util.*;
...
import org.ikov.client.graphics.ResourceLoader;
...
import org.ikov.client.accounts.Account;
import org.ikov.client.accounts.AccountManager;
import org.ikov.client.cache.Archive;
import org.ikov.client.cache.definition.Animation;
import org.ikov.client.cache.definition.Flo;
import org.ikov.client.cache.definition.IdentityKit;
import org.ikov.client.cache.definition.ItemDefinition;
import org.ikov.client.cache.definition.MobDefinition;
import org.ikov.client.cache.definition.ObjectDefinition;
import org.ikov.client.cache.definition.OverLayFlo317;
import org.ikov.client.cache.definition.SpotAnimDefinition;
import org.ikov.client.cache.definition.VarBit;
import org.ikov.client.cache.definition.Varp;
import org.ikov.client.cache.node.Deque;
import org.ikov.client.cache.node.Node;
import org.ikov.client.cache.ondemand.OnDemandFetcher;
import org.ikov.client.cache.ondemand.OnDemandRequest;
import org.ikov.client.constants.GameFrameConstants;
import org.ikov.client.constants.SizeConstants;
import org.ikov.client.constants.GameFrameConstants.GameFrameType;
import org.ikov.client.entity.player.Player;
import org.ikov.client.entity.player.PlayerHandler;
import org.ikov.client.graphics.Background;
import org.ikov.client.graphics.CursorData;
import org.ikov.client.graphics.DrawingArea;
import org.ikov.client.graphics.RSImageProducer;
import org.ikov.client.graphics.Sprite;
import org.ikov.client.graphics.SpriteCache;
import org.ikov.client.graphics.SpriteLoader;
import org.ikov.client.graphics.SpriteLoader2;
import org.ikov.client.graphics.fonts.Censor;
import org.ikov.client.graphics.fonts.RSFontSystem;
import org.ikov.client.graphics.fonts.TextClass;
import org.ikov.client.graphics.fonts.TextDrawingArea;
import org.ikov.client.graphics.fonts.TextInput;
import org.ikov.client.graphics.gameframe.GameFrame;
import org.ikov.client.graphics.gameframe.GameFrame.ScreenMode;
import org.ikov.client.graphics.gameframe.impl.ChatArea;
import org.ikov.client.graphics.gameframe.impl.MapArea;
import org.ikov.client.graphics.gameframe.impl.TabArea;
import org.ikov.client.graphics.rsinterface.DamageDealer;
import org.ikov.client.graphics.rsinterface.GrandExchange;
import org.ikov.client.graphics.rsinterface.MagicInterfaceData;
import org.ikov.client.graphics.rsinterface.PetSystem;
import org.ikov.client.io.ByteBuffer;
import org.ikov.client.io.ISAACCipher;
import org.ikov.client.net.Connection;
import org.ikov.client.net.HttpDownloadUtility;
import org.ikov.client.renderable.Animable;
import org.ikov.client.renderable.Animable_Sub3;
import org.ikov.client.renderable.Animable_Sub5;
import org.ikov.client.renderable.Entity;
import org.ikov.client.renderable.Item;
import org.ikov.client.renderable.NPC;
import org.ikov.client.renderable.PlayerProjectile;
import org.ikov.client.tools.FileUtilities;
import org.ikov.client.world.CollisionMap;
import org.ikov.client.world.CustomObjects;
import org.ikov.client.world.Model;
import org.ikov.client.world.Object1;
import org.ikov.client.world.Object2;
import org.ikov.client.world.Object3;
import org.ikov.client.world.Object5;
import org.ikov.client.world.ObjectManager;
import org.ikov.client.world.Rasterizer;
import org.ikov.client.world.Texture;
import org.ikov.client.world.WorldController;
import org.ikov.client.world.background.ScriptManager;
import org.ikov.client.world.music.Class56;
import org.ikov.client.world.music.Class56_Sub1_Sub1;
import org.ikov.client.world.sound.Class25;
import org.ikov.client.world.sound.Class3_Sub7;
import org.ikov.client.world.sound.Class3_Sub7_Sub1;
import org.ikov.client.world.sound.Class3_Sub7_Sub2;
import org.ikov.client.world.sound.Class3_Sub9_Sub1;
import org.ikov.client.world.sound.Class5;
import org.ikov.client.world.sound.Class5_Sub1;
import org.ikov.client.world.sound.Class5_Sub2;
import org.ikov.client.world.sound.Class5_Sub2_Sub1;
import org.ikov.client.world.sound.Class5_Sub2_Sub2;
import org.ikov.client.world.sound.Sound;
import org.ikov.client.world.sound.Sounds; 
import org.ikov.client.computer.*;
...
public class Client extends GameRenderer {
	...
	public String indexLocation(int cacheIndex, int index) {
		return Signlink.getCacheDirectory() + "/cache_index_data/index" + cacheIndex + "/"
				+ (index != -1 ? index + ".gz" : "");
	}
	...
	public void savePlayerData() {
		try {
			...
			if (stream != null) {
				...
				stream.writeUTF(saved_characters_usernames[0]);
				stream.writeUTF(saved_characters_usernames[1]);
				stream.writeUTF(saved_characters_usernames[2]);
				stream.writeUTF(saved_characters_passwords[0]);
				stream.writeUTF(saved_characters_passwords[1]);
				stream.writeUTF(saved_characters_passwords[2]);
	...
	private void loadPlayerData() throws IOException {
		...
		try {
			...
			saved_characters_usernames[0] = stream.readUTF();
			saved_characters_usernames[1] = stream.readUTF();
			saved_characters_usernames[2] = stream.readUTF();
			saved_characters_passwords[0] = stream.readUTF();
			saved_characters_passwords[1] = stream.readUTF();
			saved_characters_passwords[2] = stream.readUTF();
				...
				if (childInterface.type == 2) {
					...
					*/
					...
					if(childInterface.id == 13822) {
						...
						//childInterface.sprite1 = cacheSprite2[56];
	...
	public void drawLoginScreen() {
		...
		if(loginMessage1.length() > 0) {
			...
			if(bhover) {
				cacheSprite2[29].drawAdvancedSprite(347, 458);
			}
			
			if(chover) {
				cacheSprite2[30].drawAdvancedSprite(382, 458);
			}	
			...
			if(input1Hover) {
				cacheSprite2[23].drawAdvancedSprite(259, 153);
			} else {
				cacheSprite2[22].drawAdvancedSprite(259, 153);
			}	
			
			if(input2Hover) {
				cacheSprite2[23].drawAdvancedSprite(259, 203);
			} else {
				cacheSprite2[22].drawAdvancedSprite(259, 203);
			}
			...
			if(rememberMeButtonHover) {
				if(rememberMe) {
					cacheSprite2[35].drawAdvancedSprite(258, 234);
				} else {
					cacheSprite2[34].drawAdvancedSprite(258, 234);
				}
			} else {
				if(rememberMe) {
					cacheSprite2[25].drawAdvancedSprite(256, 231);
				}
			}
		...
		if(!saved_characters_usernames[1].equals("Empty")) {
			if(profileclose2hover) {
				cacheSprite2[26].drawAdvancedSprite(405, 374);
			} else {
		...
		if(!saved_characters_usernames[2].equals("Empty")) {
			if(profileclose3hover) {
				cacheSprite2[26].drawAdvancedSprite(485, 374);
			} else {
	...
	*/
	...
	public void processDrawing() {
		if (loadingError) {
			processLoadingError("An internal error occured whilst loading the Ikov client", "The client's common error quick fix system is attempting to repair the cause", "The client will automatically close in 10 seconds...");
			return;
		}
		if(isLoading)
			return;
	...
	public boolean handleRejection(int loginCode, String username, boolean reconnecting, Client client, String password) throws IOException {
		...
		if (loginCode == 6) {
			loginMessage1 = "Ikov is currently being updated.";
			loginMessage2 = "Please try again in 60 seconds..";
			return false;
		}
		if (loginCode == 7) {
			loginMessage1 = "Ikov is currently busy.";
			loginMessage2 = "Please try again.";
			return false;
		}
		if (loginCode == 8) {
			loginMessage1 = "Ikov's login server is down.";
			loginMessage2 = "Please try again in 60 seconds..";
			return false;
		}
		if (loginCode == 9) {
		...
		if (loginCode == 23) {
			loginMessage1 = "Ikov is currently being launched.";
			loginMessage2 = "Please try again in 60 seconds..";
			return false;
		}
		if (loginCode == 27) {
		...
		if(loginCode == 30) {
			loginMessage1 = "Ikov has been updated!";
	...
	private static void showErrorScreen(Client client, String... messages) {
		...
		g.drawString("Sorry, an error has occured whilst starting up Ikov client", 30, 35);
	...
	void startUp() {
		...
		try {
			for(int i = 1; i <= 3; i++) {
				if(!new File(Signlink.getCacheDirectory() + "loading"+i+".png").exists()) {
					HttpDownloadUtility.downloadFile("https://dl.dropboxusercontent.com/u/344464529/IKov/cache_screen/loading"+i+".png", Signlink.getCacheDirectory(), "loading"+i+".png");
				}
	...
	public void loadModIcons() {
		modIcons[0] = cacheSprite[828]; //Moderator
		modIcons[1] = cacheSprite[828]; //Moderator
		modIcons[2] = cacheSprite[829]; //Administrator
		modIcons[3] = cacheSprite2[3]; //Owner
		modIcons[4] = cacheSprite[837]; //Support
		modIcons[5] = cacheSprite2[2]; //YouTuber
		modIcons[6] = cacheSprite2[54]; //Global Moderator
		modIcons[7] = cacheSprite2[16]; //Regular Donator
		modIcons[8] = cacheSprite2[15]; //Super Donator
		modIcons[9] = cacheSprite2[14]; //Extreme Donator
		modIcons[10] = cacheSprite2[37]; //Legendary Donator
		modIcons[11] = cacheSprite2[38]; //Uber Donator
		modIcons[12] = cacheSprite[840]; //Regular Ironman
		modIcons[13] = cacheSprite[839]; //Hardcore Ironman
		modIcons[14] = cacheSprite2[72]; //Community Manager
		modIcons[15] = cacheSprite2[66]; //Wiki Editor
		modIcons[16] = cacheSprite2[65]; //Wiki Manager
		modIcons[17] = cacheSprite2[90]; //Staff Manager
	}
	...
	public void drawSpecOrb() {
		...
		cacheSprite2[18].drawSprite(x, y);
